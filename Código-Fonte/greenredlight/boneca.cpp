...
void Boneca::render(Camera m_camera) {
  ...
  glm::mat4 model{1.0f};
  model = glm::translate(model, glm::vec3(0.0f, 0.0f, -2.0f)); // reposicionar o centro
  model = glm::rotate(model, m_angle, glm::vec3(0, 1, 0)); // angulo de rotação e em torno de qual eixo
...
void Boneca::update(const GameData &gameData, float deltaTime) {
  if(gameData.m_state == State::Playing) {
    if(changeTimer && girarPraTras == false && girarPraFrente == false) {
      if(m_deFrenteTimer.elapsed() > 4000.0/1000 || primeiraVez) {
    ...
    if(girarPraFrente) { //só funciona se estiver virado pra tras
      if(m_angle > glm::radians(180.0f))
        m_angle = glm::wrapAngle(m_angle + glm::radians(360.0f) * deltaTime);
      if(m_angle <= glm::radians(180.0f)) {
    ...
    if(Timer==4 && timerTeste) {
      if(m_segundosTimer.elapsed() > 2000.0/1000) {
        girarPraFrente = true;
        timerTeste = false;
        changeTimer = true;
        } 
