...
void OpenGLWindow::initializeGL() {
  ...
  //m_ground.setupVAO(m_program);
  //m_boneca.setupVAO(m_program);

  // Load model
  m_ground.loadObj(getAssetsPath() + "ground.obj", false);
  m_ground.createBuffers(m_program);

  m_parede.loadObj(getAssetsPath() + "walls.obj", true);
  m_parede.createBuffers(m_program);

  m_boneca.loadObj(getAssetsPath() + "doll.obj", false);
  m_boneca.createBuffers(m_program);

  m_soldado.loadObj(getAssetsPath() + "PinkSoldier.obj", false);
  m_soldado.createBuffers(m_program);

  m_arvore.loadObj(getAssetsPath() + "tree1.obj", false);
  m_arvore.createBuffers(m_program);

  m_bonecos.loadObj(getAssetsPath() + "amongus.obj", true);
  m_bonecos.createBuffers(m_program);
...
void OpenGLWindow::paintUI() { 
  abcg::OpenGLWindow::paintUI();
  {
    ...
    ImGui::Begin(" ", nullptr, flags);
    ImGui::PushStyleColor(ImGuiCol_Text, IM_COL32(135, 230, 234, 255));
    ImGui::PushFont(m_font);
...
void OpenGLWindow::checkWinCondition() {
    if(m_camera.m_eye.z <= -1.0f) {
      m_gameData.m_state = State:: Win;
      m_restartWaitTimer.restart();
    }
}

void OpenGLWindow::checkEndGameCondition() {
...
void OpenGLWindow::restart() {
  m_camera.m_eye = {0.0f, 0.5f, 3.0f};
  m_camera.m_at = {0.0f, 0.5f, 0.0f};
